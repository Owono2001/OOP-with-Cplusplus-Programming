    ...
    string generateUserId(const string& accessLevel) {
        ...
        while (getline(file, line)) {
            ...
            if (fieldValue.find(prefix) == 0) {
                int id = stoi(fieldValue.substr(prefix.length()));
                if (id > maxId) {
                    maxId = id;
                }
            }
    ...
    void CreatePR() {
        if (!isPRIDTaken(PRID)) {
            if (isSupplierCodeValid(supplierCode) && isSalesManagerValid(salesManager)) {
                ...
                cout << "Invalid supplier code or sales manager. Purchase requisition not created." << endl;
            }
        } else {
